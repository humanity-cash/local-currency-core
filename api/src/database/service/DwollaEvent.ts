...
export async function create(input: ICreateDwollaEventDBItem): Promise<IDwollaEventDBItem> {
...
export async function findByObject(eventId: string): Promise<IDwollaEventDBItem> {
  const response = await DwollaEvent.find({eventId: eventId}, "eventId");
  if(response?.length > 0)
    return removeMongoMeta(response[0].toObject());
  else
    return undefined;
}