...
export async function mockTransferWebhook(
  ...
  createForSecondaryTransfer: boolean
): Promise<MockWebhook[]> {
  ...
  log(
    ...
    } for user are ${JSON.stringify(transfers, null, 2)}`
  );
  ...
  for (let i = 0; i < transfers?.length; i++) {
    ...
    if (type == "DEPOSIT") {
      initiatingTransfer = await getDwollaResourceFromLocation(
        process.env.DWOLLA_BASE_URL +
          "transfers/" +
          transfers[i].fundingTransferId
      );
      secondaryTransferLink =
        initiatingTransfer?.body?._links["funded-transfer"]?.href;
      if (secondaryTransferLink)
        secondaryTransfer = await getDwollaResourceFromLocation(
          secondaryTransferLink
        );
    } else {
      initiatingTransfer = await getDwollaResourceFromLocation(
        process.env.DWOLLA_BASE_URL +
          "transfers/" +
          transfers[i].fundedTransferId
      );
      secondaryTransferLink =
        initiatingTransfer?.body?._links["funding-transfer"]?.href;
      if (secondaryTransferLink)
        secondaryTransfer = await getDwollaResourceFromLocation(
          secondaryTransferLink
        );
