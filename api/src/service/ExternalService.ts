...
export async function purgeImageForUser(userId:string) : Promise<number> {
    ...
    log(`ExternalService::purgeImageForUser() purge image target URL is ${userImageUrl}`);
    ...
    log(`ExternalService::purgeImageForUser() response to purge request is ${response.status} with data ${JSON.stringify(response.data, null, 2)}`);
